...
You can create or change a change preprocessor with the *FileNet Enterprise Manager* or by writing custom code. The documentation contains a lot of code examples how to do this. Besides the configuration of the change preprocessor action handler there are not very much additional configuration operations that you can perform. The name, the description and the status (enabled or disabled) are the only configuration options.

*Figure 3. Configuring a change preprocessor.*
...
Deploying also works the same as other server side extension. If you have written your change preprocessor handler as a Java class you can either deploy the compiled Java class using a code module or by adding the compiled Java class to the class path of your application server.
...
Now that we know when to apply the unique number to the object, it is time to actually apply the unique number. Before we do that, first a reminder how this was done using an event action handler. In the previous article we used the following code to accomplish this task:
...
Just like “normal” properties, you can assign a default value to a transient property. This default value is included in the properties collection of the object just before it is about to be created, and this is exactly the situation when we want to set the value of a SOOC property. Using the *Enterprise Manager* you have to take the following steps to implement this mechanism:
* First you have to create a new string type property template called `DispenserPath`, make sure that in the properties dialog of  this property you change the value of the persistence from the default value of “System Table” to “Transient”:
...
TODO: dit nog uitwerken in code.
